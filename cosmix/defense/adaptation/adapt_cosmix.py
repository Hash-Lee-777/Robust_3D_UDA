...
from utils.defense.defense_lightning import SimMaskedAdaptationDefense
...
# unfinished
def load_decoder(checkpoint_path,decoder):
    return decoder
...
def adapt(config):
    ...
    source_training_dataset, source_validation_dataset, _ = get_dataset(
        dataset_name=config.source_dataset.name,
    ...
    source_validation_dataloader = get_dataloader(
    ...
    target_validation_dataloader = get_dataloader(
    ...
    Decoder = getattr(models, config.model.decoder_name)
    decoder = Decoder(config.decoder.in_feat_size, config.decoder.out_classes)
    decoder = ME.MinkowskiBatchNorm.convery_sync_batchnorm(decoder)
