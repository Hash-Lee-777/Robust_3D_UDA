...
from utils.losses import CELoss, DICELoss, SoftDICELoss, KPSLoss, CombinedLoss
import pytorch_lightning as pl
from sklearn.metrics import jaccard_score
import open3d as o3d
import pdb
import utils.datasets.synlidar
...
class PLTTrainer(pl.core.LightningModule):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.cls_num_list = [1]*num_classes
        ...
        elif criterion == 'KPSLoss':
            if self.num_classes == 19:
                self.criterion = KPSLoss(cls_num_list=self.cls_num_list, is_kitti=True)
            else:
                self.criterion = KPSLoss(cls_num_list=self.cls_num_list, is_kitti=False)
        elif criterion == 'CombinedLoss':
            self.criterion = CombinedLoss(self.soft_dice_params, self.kps_params, self.lambda_weight)
        else:
    ...
    def training_step(self, batch, batch_idx):
        self.cls_num_list = self.count_cls_num_list(batch['labels'],self.num_classes)
