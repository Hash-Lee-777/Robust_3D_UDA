...
class ConcatDataset(Dataset):
    ...
    def merge(self, source_data, target_data):
        ...
        self.data = {**source_data, **target_data} 
    ...
    def __getitem__(self, idx:int):
        ...
        if idx < len(self.source_dataset):
            ...
            new_idx = np.random.choice(len(self.source_dataset),1)
            source_data = self.source_dataset[new_idx]
    ...
    # how to get dict data
    def get_cls_num_list(self, i: int):
        if i < len(self.source_dataset):
            source_data = self.source_dataset[i]
        else:
            new_idx = np.random.choice(len(self.source_dataset),1)
            source_data = self.source_dataset[new_idx]

        target_data = self.target_dataset.get_data(i)
        self.data = self.merge(source_data, target_data)
        source_labels = self.data['source_labels']
        target_labels = self.data['target_labels']
        # 合并两个标签数组
        all_labels = torch.cat((source_labels, target_labels))
        
        # 统计每个标签的数量（忽略值为 -1 的标签）
        counts = torch.bincount(all_labels[all_labels != -1], minlength=19)
        return counts
