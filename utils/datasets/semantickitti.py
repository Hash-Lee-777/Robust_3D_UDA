...
class SemanticKITTIDataset(BaseDataset):
    ...
    #     unique_labels, cls_num_list = np.unique(label, return_counts=True)
    #     label_cls=dict(zip(unique_labels,cls_num_list))
    #     return label_cls
    
    # def get_cls_num_list(self, i: int):

        cls = self.get_label_cls(i)
        if -1 in cls:
            del cls[-1]
        num_classes = 19
        cls_num_list = [0] * num_classes
        for cls, count in cls.items():
