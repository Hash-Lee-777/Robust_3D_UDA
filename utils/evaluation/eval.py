...
def test(config, resume_checkpoint):
    ...
    _, validation_dataset, target_dataset = get_dataset(
    ...
    msg= f'[INFO] choose target dataset for evaluation!' if args.eval_target else f'[INFO] choose source validation dataset for evaluation!'
    print(msg)
...
def test_with_config(config,resume_checkpoint):
    def get_dataloader(dataset, shuffle=False, pin_memory=True):
        return DataLoader(dataset,
                          batch_size=config.pipeline.dataloader.batch_size*4,
                       collate_fn=CollateFN(),
    ...
    dataset = validation_dataset if config.eval.eval_source else target_dataset

    msg= f'[INFO] choose target dataset for evaluation!' if config.eval.eval_target else f'[INFO] choose source validation dataset for evaluation!'
    print(msg)
    ...
    if config.eval.eval_source:
        tester.test(plt_model, dataloaders=validation_dataloader, verbose=False)
    elif config.eval.eval_target:
        tester.test(plt_model, dataloaders=target_dataloader, verbose=False)
    else:
        print('Not evaluating!')

def multiple_test(config, path=None):
    if path is None:
        path = config.eval.resume_path
        ...
        for checkpoint in list_checkpoint:
            if not os.path.isfile(os.path.join(path, 'checkpoints', 'evaluation', 'results', checkpoint[:-5]+'_test.csv')):
                ...
                test_with_config(config, os.path.join(path, 'checkpoints', checkpoint))
            else:
