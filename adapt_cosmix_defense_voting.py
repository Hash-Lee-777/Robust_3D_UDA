...
from utils.defense.cosmix.defense_lightning_voting import SimMaskedAdaptationDefense
from utils.common.momentum import MomentumUpdater
import yaml
from attack import AdvSynLidarTrain,AdvSynlidarVal
import sys,os
...
parser.add_argument("--config_file",
                    default="configs/defense/cosmix/lidar2kitti_defense.yaml",
                    type=str,
...
# unfinished
def load_decoder(checkpoint_path=None,decoder=None):
    return decoder if checkpoint_path is None else decoder.load_state_dict(torch.load(checkpoint_path)['state_dict'], strict=False)
...
def adapt(config):
    ...
    try:
        source_mapping_path = 'utils/datasets/'+config.source_dataset.mapping_path
    ...
    try:
        target_mapping_path = 'utils/datasets/'+config.target_dataset.mapping_path
    ...
    # source_training_dataset, source_validation_dataset
    source_training_dataset = AdvSynLidarTrain(
        version=config.source_dataset.version,
        phase='train',
        dataset_path=config.source_dataset.dataset_path,
        adv_dataset_path=config.attack.adv_pcd_path,
        adv_gt_label=False,
        ...
        attacked_prob=config.attack.attack_prob,   # For clean data set this to 0 in the config.
        split_path=config.defense.split_path
        )
    
    source_validation_dataset = AdvSynlidarVal(
        version=config.source_dataset.version,
        phase='validation',
        dataset_path=config.source_dataset.dataset_path,
        adv_dataset_path=config.attack.adv_pcd_path,
    ...
    # selecting clean synlidar dataset for augment
    # for test
    subset_size = config.defense.gt_scene
    gt_training_dataset = AdvSynLidarTrain(
        version=config.source_dataset.version,
        phase='train',
        dataset_path=config.source_dataset.dataset_path,
        adv_dataset_path=config.attack.adv_pcd_path,
        ...
        num_classes=19,
        ignore_label=config.source_dataset.ignore_label,
        attacked_prob=0.,   # clean data
        split_path=config.defense.split_path
    )
    subset_indices = random.sample(range(len(gt_training_dataset)),subset_size)
    gt_training_dataset = Subset(gt_training_dataset,subset_indices)

    target_training_dataset, target_validation_dataset, _ = get_dataset(
    ...
    # Decoder = getattr(models, config.defense.decoder.name)
    # decoder = Decoder(config.defense.decoder.in_feat_size, config.defense.decoder.out_classes)
    # decoder = ME.MinkowskiSyncBatchNorm.convert_sync_batchnorm(decoder)
    # Load VRC decoder
    defense_config = get_config(config.defense.cfg)
    model_module = importlib.import_module(f'VRCNet.models.{defense_config.model_name}')
    decoder = model_module.Model(defense_config)
    decoder.load_state_dict(
        torch.load(defense_config.load_model,map_location=torch.device('cpu'))['net_state_dict']
    )
    # Randomize decoder
    ...
    pl_module = SimMaskedAdaptationDefense(
        decoder=decoder,
        ...
        decode_coords=config.defense.decode_flag,
        conf_update=config.defense.conf_update,
        update_pts=config.defense.update_pts,
        lambda_weight=config.adaptation.lambda_weight,
        gt_training_dataset=gt_training_dataset
    )
