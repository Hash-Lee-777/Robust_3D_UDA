...
from attack.datasets.SynlidarPhase import *
from attack.datasets.AdvSynlidar import *
...
parser.add_argument("--config_file",
                default="configs/attack/warmup/lidar2kitti_advTrain.yaml",
                type=str,
...
if __name__ == '__main__':
    ...
    adv_training_dataset = AdvSynLidarTrain(
        version=config.dataset.version,
        phase='train',
        dataset_path=config.dataset.dataset_path,
        adv_dataset_path=config.attacked.adv_dataset_path,
        adv_gt_label=config.attacked.train.gt,   #False
        ...
        attacked_prob=config.attacked.train.prob, #1.
    )
    
    adv_validation_dataset = AdvSynlidarVal(
        version=config.dataset.version,
        phase='validation',
        dataset_path=config.dataset.dataset_path,
        adv_dataset_path=config.attacked.adv_dataset_path,
        adv_gt_label=config.attacked.validation.gt, #False
        mapping_path=mapping_path,
        voxel_size=config.dataset.voxel_size,
        augment_data=False,
        sub_num=config.dataset.num_pts,
        num_classes=config.model.out_classes,
        ignore_label=config.dataset.ignore_label,
        attacked_prob=config.attacked.validation.prob, #0.
    )
    
    print(f'[INFO] Initialize adversarial training and validation dataset.')
    ...
    # print(f'[INFO] USING GROUND TRUTH LABEL {config.attacked.gt}!')
    trainer.fit(pl_module,
                train_dataloaders=training_dataloader,
                val_dataloaders=validation_dataloader)
    print(f"[INFO] WARM UP STEP FINISHED!")
    print(f'[INFO] PRETRAINED MODEL SAVED WEIGHT IS AT {save_dir}')
